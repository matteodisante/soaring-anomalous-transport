...
Reads the ``flights_meta.parquet`` that ``scripts/pipeline/preprocess.py`` writes -- one
row per flight *attempted*, dropped ones included -- and emits
...
Run it after ``scripts/pipeline/preprocess.py``, with the same data roots::

    SOARING_PARA_DATA_ROOT=... SOARING_DELTA_DATA_ROOT=... \
    uv run python scripts/tesi/ch02_dataset/generate_pipeline_census.py
"""
