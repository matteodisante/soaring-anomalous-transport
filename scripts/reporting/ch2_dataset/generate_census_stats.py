...
the thesis build. Run it after refreshing the scan caches::
    ...
    uv run python scripts/reporting/ch2_dataset/generate_census_stats.py
"""
...
def _placeholder_date_macros(prefix: str, catalog) -> dict[str, str]:
    ...
    Args:
        ...
        catalog: The per-flight catalog table (``date``, and ``season`` where present).

    Returns:
        Mapping ``macro name -> value`` for the sentinel-date census.
    """
    date = catalog["date"].astype(str)
    bad = date.str.startswith("0000")
    out = {
    ...
    if bad.any() and "season" in catalog.columns:
        affected = catalog.loc[bad, "season"].value_counts()
        total = catalog["season"].value_counts().reindex(affected.index)
        share = (affected / total).sort_values(ascending=False)
        worst_season = str(share.index[0])
        worst_share = 100.0 * float(share.iloc[0])
    out[f"StatCat{prefix}PlaceholderWorstSeason"] = worst_season
    out[f"StatCat{prefix}PlaceholderWorstPct"] = _fmt(worst_share, 2)
    return out
