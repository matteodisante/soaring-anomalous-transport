...
def load(discipline: str):
    ...
    try:
        ...
        return None
    meta_path = cfg.derived_dir / "flights_meta.parquet"
    if not meta_path.is_file():
        return None

    meta = pd.read_parquet(meta_path)
    catalog_path = Path(cfg.catalog_path) if cfg.catalog_path else None
...
def main() -> int:
    ...
    for discipline in DISCIPLINES:
        ...
        if meta is None:
            print(f"{discipline}: flights_meta not reachable, skipping")
            continue
    ...
    lines = ["% Generated by scripts/reporting/generate_dataset_stats.py -- do not edit."]
