...
#   7. audit    -- one more streaming pass, keeping each flight's position at every lag
...
#                  step 3.
#   8. report   -- the \StatAudit* macros, reduced from step 7 and from the curve CSV.
#   9. prelim   -- Sec. 2.8: the retained-ensemble figures and the \StatPrelim* macros.
#                  Reads step 7's arrays, not the fix table, which is what makes a
#                  stratified MSD a row selection rather than another 43 GB scan.
#  10. dataset  -- the discard cascade with its running remainder, and the survivors
#                  per season. Reads flights_meta and the catalogue; seconds.
#  11. variations -- the third full traversal, keeping one filtered-variation curve per
#                  flight per filter order (and, at orders 1-2, per component too). ~25
#                  min. It is what lets step 12 stratify by cadence, wing class, season
#                  and task without touching the fix table.
#  12. transport -- Chapter 3's measurement: the order scan (pooled and per component),
#                  its uncertainty, the stratifications and the regime fit. Minutes.
#  13. shape    -- the fourth traversal: the increments themselves, for the moment
#                  spectrum and the velocity memory. A full pass over the fix table.
#  14. shapefig -- their reduction into tab:shape and \StatShape*: the tail exponent of
#                  the autocorrelation, and the matched Gaussian null the non-Gaussianity
#                  is read against.
#  15. propagator -- the fifth traversal: histograms of the increments per lag, per
...
#  16. edge     -- the sixth traversal, and the only subsampled one: the ensemble MSD
...
#  17. circling -- the velocity autocorrelation at native cadence on the 1 Hz segments. The
...
#  18. contract -- every macro the thesis quotes must now exist. An undefined one inside
...
step "16/19  edge-sample effect -> StatEdge*, the size of what the MSD reads and the verifier does not"
"$PY" scripts/reporting/ch3_global_transport/measure_edge_effect.py

step "17/19  circling -> StatCircling*, the VACF at native cadence, below the shape pass's grid"
"$PY" scripts/reporting/ch3_global_transport/measure_circling.py

step "18/19  macro contract: everything the thesis quotes must exist"
