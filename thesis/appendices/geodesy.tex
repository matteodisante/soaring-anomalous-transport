...
\end{equation}
which is the expression quoted in Sec.~\ref{sec:enu}. It is largest where $\sin2\varphi=1$,
i.e.\ exactly at $\varphi=45^\circ$, and there equals $e^2/2=3.347\times10^{-3}$ radian, or
...
the poles and is symmetric about $45^\circ$.
\end{revblock}
...
\paragraph{Local radius of curvature.}
For a short step of angular length $d\sigma$ at geodetic latitude $\varphi$ and azimuth
$\alpha$ (measured from north), the ellipsoidal arc length is $ds=R(\varphi,\alpha)\,d\sigma$
to leading order, with $R(\varphi,\alpha)$ the local (Euler) radius of curvature,
\begin{equation}
\label{eq:geo-euler}
\frac{1}{R(\varphi,\alpha)}=\frac{\cos^2\alpha}{M(\varphi)}+\frac{\sin^2\alpha}{N(\varphi)},
\end{equation}
...
to within \SI{10}{\metre}, so the fractional error of a step in direction $\alpha$,
$[R(\varphi,\alpha)-R_m]/R_m$, is bounded between the values it takes at $\alpha=0$ and
...
$M=N$ by symmetry) --- which is why Sec.~\ref{sec:fixlevel} calls $\lesssim0.3\%$ ``the
flattening scale'' rather than a fixed number.
...
\end{equation}
(exactly, $N(45^\circ)=\SI{6388.83}{\kilo\metre}$ and $M(45^\circ)=\SI{6367.38}{\kilo\metre}$
against $R_m=\SI{6371.01}{\kilo\metre}$): a step is stretched by at most
$\approx\SI{0.28}{\percent}$ running east--west and shortened by at most
$\approx\SI{0.06}{\percent}$ running north--south, the $\lesssim\SI{0.3}{\percent}$ bound
quoted in Sec.~\ref{sec:fixlevel}, about \SI{3}{\centi\metre} on a \SI{10}{\metre} step. The
...
still two orders of magnitude below the metre-scale GPS noise on the same step.
\end{revblock}
