...
\label{impl:global}
\rev{This chapter mirrors Chapter~\ref{ch:global}. Of it, the two mean-squared-displacement
estimators and their audit are built, and Sec.~\ref{impl:obs-global} is a populated record
of them; the remaining observables are stated in the body and not yet implemented.}
...
both are pinned by \path{tests/analysis/observables/test_moments.py}: an exact fractional
Brownian motion returns \num{0.01} in the median over the same lag grid, and a Lévy walk of
tail index \num{1.5} returns \num{0.56}. It is quoted over
...
taking a Kaplan--Meier median instead returns \num{1.95} and \num{2.03}, inside the quoted
interval --- but refitting over sub-ranges of radius moves that between \num{1.51} and
\num{2.03}, a spread five times the uncertainty it was meant to check.
The reason is the one this chapter already gives for the ensemble MSD. $T(L)$ is measured
