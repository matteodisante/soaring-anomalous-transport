...
replaces them as a source of exponents, the moment spectrum, the non-Gaussian parameter, and
the velocity autocorrelation and its tail. One observable the body catalogues
is defined and not measured, and Sec.~\ref{sec:transport-kinematics} says which.}
...
unit-tested, as is every other observable Chapter~\ref{ch:global} defines except the return
probability, which Sec.~\ref{sec:transport-kinematics} names as the one gap. The
estimators live in
...
$\alpha_2^{\mathrm{NG}}=\langle|\Delta\mathbf{r}|^4\rangle/2\langle|\Delta\mathbf{r}|^2\rangle^2-1$ at
...
of \num{0.96} for a Lévy walk of index \num{1.5}. A Lévy walk of index \num{1.5} leaves a departure of about \num{0.084} on this grid; the
archive leaves \StatShapeParaLinearDeparture\ and \StatShapeHangLinearDeparture, which is
roughly eight and fourteen times smaller, and about half and a third of the declaration
threshold. The margin against a knee is therefore an order of magnitude; the margin against
the gate is a factor of two, and the two should not be confused.}
...
season and declared task. Every stratification of Sec.~\ref{sec:transport-measure} is then a
...
\paragraph{\rev{Why the runs test is not read against a normal table.}} \rev{The
Wald--Wolfowitz statistic is asymptotically normal under a null of \emph{independent} signs,
and on a curve every lag of which averages the same flights the signs are nothing of the
kind: the measured lag-1 residual autocorrelation is \numrange{0.89}{0.95} on all four
curves. Simulating an exact power law observed with AR(1) noise of that correlation on the
same forty-lag grid, the rule $|z|>2$ rejects it in \SI{84}{\percent} of trials at
$\rho=0.8$ and \SI{90}{\percent} at $\rho=0.9$, and the true five-per-cent threshold is
$|z|$ near \num{5.1}. The $z$ values this archive returns, \numrange{-4.9}{-5.8}, are
therefore not significances, and \path{residual_runs} now says so in its own docstring.}

\rev{Calibrating the null on the \emph{observed} residual autocorrelation would be circular in
the way this project has been caught by once already, since a genuinely bent curve has a
high residual autocorrelation because of the bend. So the conclusion rests on a different
comparison, \path{residual_against_noise}: the residual against the standard error of each
lag's own mean, taken from the stored \SIrange{10}{90}{\percent} band and count on a normal
equivalence, which understates a right-skewed spread and so returns a lower bound. It gives
\numrange{22}{51} across the four curves --- a departure that many times the error on each
point cannot be scatter, whatever its arrangement.}
