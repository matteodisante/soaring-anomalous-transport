...
\label{impl:glider}
The placeholder class is the literal \enquote{0} in \path{aile_class}; where
...
\path{soaring.analysis.preprocessing} as it walks a flight's fixes once:
\begin{itemize}[nosep]
...
absolute bound (no context). Status: Table~\ref{tab:impl-status} -- the detectors
...
squares, subtract it, and compare the range of the residual against
\texttt{ground\_flatness\_m}. The linear term is the barometric drift, which over a stint
of $T_{\mathrm{ground}}$ is already of the order of the tolerance; the residual is what is
left of genuine vertical motion. No new config key is needed --- the tolerance now applies
to the residual rather than to the raw range, and that is the whole change.} \flow{Below
...
\begin{itemize}[nosep]
  ...
  \item \path{soaring.analysis.preprocessing.track_stats} reduces those fixes to the one
...
so every count in Sec.~\ref{sec:flightfilter} is recomputed once the fix-level stage
exists, and should be read as the present best estimate rather than as a final figure.
\end{revblock}
