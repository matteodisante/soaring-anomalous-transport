...
\begin{tabular}{|p{4.6cm}|l|p{6.1cm}|}
\hline
...
\noindent (abridged: the full forty-one are listed in Sec.~\ref{impl:onDisk}.) A dropped flight
...
Chapter~\ref{ch:next-steps} being wrong, which is the expensive way to find a bug, and
...
air holds a ground speed near zero, gains \SI{700}{\metre} over a $T_{\mathrm{ground}}$
...
\label{impl:prelim}
\begin{revblock}
\path{scripts/reporting/generate_prelim_figure.py} produces both figures of
Sec.~\ref{sec:prelim} and the \texttt{\textbackslash StatPrelim*} macros. It reads three
