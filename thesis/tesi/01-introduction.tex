...
The empirical programme therefore calls for measurements of the MSD, displacement
quantiles and directional anisotropy, and how each depends on route, duration and
equipment.
Quantile ratios test whether the distribution changes shape as the lag increases. Open
and closed tasks help expose the effect of the flight's intended geometry. These
measurements, together with the spectrum of moments, the shape of the increment
distribution and velocity correlations, must be compared with explicit model predictions: higher
moments compare the growth of ordinary and large displacements, while velocity correlations
resolve persistence in time. A model is challenged where its predictions disagree with these
observations under a comparable measurement protocol.
...
legs~\cite{zaburdaev2015}. Each comparison must specify the hypotheses being tested
and leave unsupported exclusions open.
...
of persistence.
A probabilistic segmentation into transition, search and climb can address this
missing link. It requires an explicit mathematical model, readable trajectory
examples, and a distinction between internal consistency and accuracy against
...
The present document contains the introduction, the data chapter and the analysis of
horizontal transport and displacement scaling, together with the two appendices
cited in Chapter~2. Flight-phase segmentation, conditional observables, social flight,
stochastic modelling and route optimisation form the subsequent research programme.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[schematic, node distance=5.5mm,
    ...
    \node[done, below=of data] (global) {{\bfseries Chapter 3: horizontal transport and scaling}\\[2pt]
      {\footnotesize\color{black!70} Scaling, quantile shape and environmental geometry}};
    \node[future, below=of global] (phases) {{\bfseries Flight phases}\\[2pt]
    ...
    \draw[decorate, decoration={brace, amplitude=4pt}, black!45, line width=0.6pt]
      ($(data.north west)+(-3mm,0)$) -- ($(global.south west)+(-3mm,0)$);
    \node[fig note, rotate=90, anchor=south]
      at ($($(data.north west)!0.5!(global.south west)$)+(-6mm,0)$) {this document};
    \draw[decorate, decoration={brace, amplitude=4pt}, black!45, line width=0.6pt]
      ($(phases.north west)+(-3mm,0)$) -- ($(routes.south west)+(-3mm,0)$);
    \node[fig note, rotate=90, anchor=south]
      at ($($(phases.north west)!0.5!(routes.south west)$)+(-6mm,0)$) {subsequent work};
  \end{tikzpicture}
  \caption{Logical structure of the thesis. The solid boxes are developed in this document;
...
including the treatment of missing or inconsistent channels. The two appendices
state the spectral conventions and the geodetic transformation used in that chapter.
Chapter~\ref{ch:fixed-transport} measures horizontal transport and distributional scaling
with fixed flight populations, and resolves circuit and launch-altitude differences.
...
work: the present document establishes the cleaned trajectories and the global
transport statistics that those comparisons must explain.
