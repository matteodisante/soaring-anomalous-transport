...
The median has a deeper minimum near $10^2$ s. When its local exponent is below
the mean's, the mean-to-median ratio increases; when it exceeds the mean's during
the recovery, that ratio decreases.
...
support this timescale: search times have heavy tails, whereas climb times have
an exponential tail. Their survival probabilities remain of order $10^{-3}$ at
durations of several hundred seconds. An archive with many thousands of episodes
can therefore contain several such long events alongside much shorter ones;
$10^2$--$10^3$ s is a range of observable durations, not a strict upper limit.
