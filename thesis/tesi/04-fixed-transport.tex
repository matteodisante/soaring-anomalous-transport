...
\begin{figure}[htbp]
 ...
 describe the initial cleaned fix. Circuit proportions differ between classes;
 these curves do not isolate an orographic effect. Figure~\ref{fig:fixed-task}
...
of the smaller exponent. This quantifies a difference between the pooled
populations, rather than the magnitude of an orographic effect.
...
of a fixed population into circuit types, equipment groups used as a proxy for
experience, or combinations of the two. For the full cohort, the partition must
also retain unclassified flights and any other equipment categories. If group
$g$ contains $N_g$ of the $N=\sum_g N_g$ flights, its fraction $p_g=N_g/N$ is
constant across lags. The pooled MSD and its local exponent obey
\begin{equation}
 \begin{aligned}
 M_2(\tau)&=\sum_g p_g M_{2,g}(\tau),\\
 H_{\rm loc}(\tau)&=\sum_g w_g(\tau)H_{{\rm loc},g}(\tau),
 \qquad w_g(\tau)=\frac{p_gM_{2,g}(\tau)}{M_2(\tau)}.
 \end{aligned}
 \label{eq:fixed-mixture}
\end{equation}
The second line follows by taking the logarithmic derivative of the first,
using the definition of $H_{\rm loc}$ in Eq.~\eqref{eq:fixed-local-h}.
The MSD averages the group curves with flight fractions $p_g$. The local
exponent instead weights each group by its share $w_g(\tau)$ of the pooled MSD:
a group contributes more when it contains more flights or has a larger MSD at
that lag. Even though $p_g$ is fixed, $w_g(\tau)$ changes if the group curves
grow at different rates. This identity concerns the local exponents; the fitted
$H$ summarises the growth of the pooled curve over the chosen lag interval.

Within each altitude class, open flights therefore gain weight in the pooled
local exponent both through their numbers and through their larger MSD.
...
fractions $p_g$ were matched between classes, the slope weights $w_g(\tau)$
would still depend on the MSD of each group. Open routes with larger MSDs would
therefore carry greater slope weight even with equal flight counts.
