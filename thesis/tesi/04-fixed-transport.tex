...
$H=\ChThreeHangGeneralHurst$ for hang gliders. These summarise growth with changing
flight support. The hang-glider fit reaches a tail supported by only
\ChThreeHangGeneralGroups{} groups; its interval uses \ChThreeHangGeneralReplicates{}
replicates with complete lag support.
\begin{table}[htbp]
 \centering\small
...
Section~\ref{sec:fixed-scaling}. Figure~\ref{fig:fixed-general-slopes} shows the
resulting local slopes of both general curves.

The origin-distance means are approximately ballistic at the earliest resolved lags.
Their local slopes reach minima at \ChThreeParaLocalMinLag{} s
...
\paragraph{Uncertainty.} The shading around the origin-distance curves of
...
From the eligible archive's $G$ site--day groups, each of $B=1000$ replicates
...
\begin{equation}
 ...
 M_2^{(b)}(\tau)&=\sum_{f\in\mathcal C(\tau)}\sum_{i\in\mathcal I_f(\tau)}
