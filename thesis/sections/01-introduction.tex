...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
    box/.style={draw=figblue,rounded corners=2pt,align=center,text width=11.6cm,
                inner sep=7pt,font=\small},
    future/.style={box,draw=gray,dashed},
    >=Latex,node distance=6mm]
    \node[box] (data) {\textbf{Chapter 2: trustworthy trajectories}\\
      Recorder channels, cleaning, coverage and retained uncertainty};
    \node[box,below=of data] (global) {\textbf{Chapter 3: global observables}\\
      Scaling, distribution shape, memory and environmental geometry};
    \node[box,below=of global] (phases) {\textbf{Chapter 4: flight phases}\\
      Explicit segmentation model, diagnostic figures and independent validation};
    \node[future,below=of phases] (conditional) {\textbf{Observables conditioned on phase; solo--group comparison}\\
      Mechanisms within episodes, dependencies between episodes, social information};
    \node[future,below=of conditional] (model) {\textbf{Stochastic model and Monte Carlo}\\
      Joint reproduction of global and conditional observables on held-out data};
    \node[future,below=of model] (routes) {\textbf{Thermal landscapes and route optimisation}\\
      Lift availability, wind, altitude budget and possible terrain constraints};
    \foreach \a/\b in {data/global,global/phases,phases/conditional,conditional/model,model/routes}
      \draw[->,thick,gray] (\a)--(\b);
  \end{tikzpicture}
