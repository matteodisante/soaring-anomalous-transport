...
\begin{description}
  ...
  \item[Propagator.] Since each flight starts at the origin,
        ...
        The marginal $P(x,t)$ is the most practical object to estimate from data and to
        collapse onto the scaling form~\eqref{eq:marginal-scaling} by rescaling the axis by
        $t^H$.

        \smallskip
        \textbf{Isotropy is an assumption, not a given, and must be tested.}
        Competition flights have preferred directions (race goal, orography, prevailing
        winds), so isotropy may fail. Four complementary tests are available directly from
        the data:
        \begin{enumerate}[label=(\roman*)]
          \item \emph{Variance ratio.} For an isotropic process
                $\mathrm{Var}(E(t))=\mathrm{Var}(N(t))=\mathrm{MSD}(t)/2$ for all $t$.
                The ratio $\mathrm{Var}(E(t))/\mathrm{Var}(N(t))$ should be identically $1$.
          \item \emph{Marginal comparison.} The distributions $P(E(t)=x)$ and $P(N(t)=x)$
                should coincide for all $t$; a Kolmogorov--Smirnov test provides a formal
                check.
          \item \emph{Cross-covariance.} For an isotropic process
                $\mathrm{Cov}(E(t),N(t))=0$; a non-zero value signals a preferred direction.
          \item \emph{PCA of the covariance matrix.} Diagonalise
                $\Sigma(t)=\langle\mathbf{r}(t)\mathbf{r}(t)^\top\rangle$. The eigenvalue
                ratio $\lambda_1(t)/\lambda_2(t)$ measures anisotropy ($=1$ if isotropic);
                the eigenvectors reveal the dominant axis.
        \end{enumerate}
        If isotropy fails, the derivation above is no longer valid: the two components may
        have different scaling functions or even different exponents. In that case one works
        with each component separately, or rotates to the principal axes of $\Sigma(t)$
        before applying the scaling collapse.
