...
\begin{description}
  \item[Mean-squared displacement (MSD).] With every flight starting at the origin
        ...
        fixed $t$ -- no average over a moving time origin is involved here. Its growth law
  ...
  \item[Time- versus ensemble-averaged MSD.] The \emph{time-averaged} MSD is instead built
        \emph{within} a single flight of duration $T$, by sliding the origin $t_0$ along the
        track,
        \begin{equation}
          \overline{\delta^2}(t;T)=\frac{1}{T-t}\int_0^{T-t}
          \big|\mathbf{r}(t_0+t)-\mathbf{r}(t_0)\big|^2\,\mathrm{d}t_0 .
        \end{equation}
        This is where the average over $t_0$ enters. A single flight is too short and noisy to
        trust on its own -- hence the question is well posed only at the ensemble level: we
        average the per-flight curves $\overline{\delta^2}(t;T)$ over many flights to obtain a
        robust mean time-averaged MSD, and compare it with $\mathrm{MSD}(t)$. For an ergodic
        process the two coincide at long $T$; a persistent gap, with $\overline{\delta^2}$
        depending on $T$, signals \emph{ergodicity breaking} and aging -- hallmarks of
        trapping (CTRW-type) dynamics -- and thus discriminates between candidate models.
