...
\begin{description}
  ...
  \item[Radius of gyration.] The root-mean-square distance of the trajectory points from their
        ...
        measured as it grows with elapsed time, $R_g(t)\sim t^{H}$, it provides a further
        estimate of the same exponent.
\end{description}

\paragraph{Per-flight scalars and scale dependence.}
The geometric quantities above can be read in two complementary ways: as a single number
per flight (one tortuosity, one $d_f$, one $R_g$), whose distribution over the ensemble
characterises the population and feeds the stratified comparisons; or as a function of
scale or elapsed time ($R_g(t)$ as the trajectory unfolds, tortuosity on sub-segments of
growing duration, the box count as a function of box size), which is where the transport
exponents appear ($R_g(t)\sim t^H$; the box-count slope is $d_f$). The scalar is
descriptive; the scale dependence links the geometry to the anomalous exponent. Both views
will be used.
