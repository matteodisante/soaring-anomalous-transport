...
The goal is to remove individual fixes that are physically impossible or corrupted, without
discarding whole flights. The criteria are applied in sequence, each removing fixes that
survive all previous steps.

\begin{enumerate}[label=(\arabic*)]
  \item \emph{Validity flag.} Drop any B record with validity code \texttt{V} (2-D fix,
        no GPS altitude, degraded horizontal accuracy); retain only \texttt{A} (3-D fix).
  \item \emph{Duplicate timestamps.} If two consecutive records share the same UTC second,
        keep the first and discard the duplicate.
  \item \emph{Altitude range.} Drop fixes with GPS altitude outside a physically plausible
        range for soaring in France (e.g.\ $[-100,\,6000]$\,m); extreme values indicate
        receiver faults.
  \item \emph{Horizontal speed.} Compute the horizontal distance between consecutive valid
        fixes divided by their time interval; if this implied speed exceeds a glider-class
        maximum (e.g.\ $\sim\!120$\,m/s for a sailplane, $\sim\!50$\,m/s for a paraglider),
        the later fix is a GPS jump and is removed.
  \item \emph{Vertical speed.} Similarly, if the altitude difference between consecutive
        fixes implies a vertical speed above a physical bound (e.g.\ $|w|>30$\,m/s), the
        fix is discarded. This criterion depends on the chosen altitude channel and is one
        more reason to make that choice explicit.
  \item \emph{Time-gap flagging.} If the interval between two consecutive valid fixes
        exceeds a threshold (e.g.\ 60\,s), the gap is flagged. Short gaps can be
        interpolated; long ones split the trajectory into independent segments or disqualify
...
\begin{enumerate}[label=(\arabic*)]
  \item \emph{Minimum number of valid fixes.} After cleaning, flights with fewer than a
        minimum number of valid fixes (e.g.\ 200) cannot support reliable estimation of
        velocity-derived quantities; they are excluded.
  \item \emph{Minimum airborne duration.} The MSD at lag $t$ requires trajectories
        of duration $T \gg t$. A minimum duration $T_{\min}$ is set relative to the maximum
        lag of interest; it must be documented explicitly, as it determines which flights
        contribute to each portion of the MSD curve. Note that $T_{\min}$ should not be set
        so large as to exclude a majority of flights: the effective sample size decreases
        with $T_{\min}$ and the surviving population becomes increasingly unrepresentative.
  \item \emph{Minimum spatial extent.} Flights that remain within a very small area
        (e.g.\ net displacement $< 1$\,km throughout) are likely tow flights, aborted
        attempts, or data artifacts, and do not constitute soaring trajectories.
  \item \emph{Glider type.} For stratified analysis by glider category (paraglider, hang
        glider, sailplane), flights without a declared or recognisable type are excluded from
        the category-specific sub-analyses but retained in the pooled analysis.
\end{enumerate}

All threshold values are \emph{analysis parameters}, not ground truths. A robustness check
-- repeating the main results (MSD exponent, distribution tails) with the thresholds varied
within physically plausible ranges -- is part of the analysis and must be reported. If the
conclusions are stable across reasonable threshold choices, the filtering is robust; if they
are sensitive, the sensitivity must be disclosed and understood.
...
dataset actually contains before any aggregate statistic is formed.
...
\paragraph{Compatibility across seasons and glider types.}
The dataset spans more than two decades and three glider categories (paraglider, hang
glider, sailplane) with very different kinematics. Before treating all flights as draws from
a single statistical process -- i.e.\ before computing ensemble averages over the full
dataset -- one must check that the subpopulations are statistically compatible.

A lightweight way to do this: for each stratum of interest (e.g.\ paragliders only,
sailplanes only, or the flights of a single season), compute $\langle E(t)^2\rangle$ as a
function of $t$ and overlay all the curves on the same plot. If they coincide, the
transport statistics are compatible across strata and pooling is valid. If one category
shows systematically larger (or differently shaped) growth -- sailplanes, for instance,
cruise much faster than paragliders -- the strata must be analysed separately, and the
agreement or disagreement is itself a scientific result about the universality of the
transport regime across glider classes.
