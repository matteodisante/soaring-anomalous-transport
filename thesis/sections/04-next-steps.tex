...
\paragraph{Stationarity across seasons and glider types.}
The dataset spans more than two decades and three glider categories (paraglider, hang
glider, sailplane) with very different kinematics. Before pooling all flights into a single
ensemble one should check that the basic statistics are compatible. A lightweight check:
compare the per-component variance $\langle E(t)^2\rangle$ as a function of $t$, stratified
by season and by glider type. Systematic differences would indicate that the transport
statistics are not universal -- which is itself a result -- and that the main analysis must
be run separately per stratum rather than on the pooled dataset.
