...
\begin{description}
  ...
  \item[Time- versus ensemble-averaged MSD.] For a single flight $k$ of duration $T_k$,
        define the \emph{time-averaged} MSD at lag $t$ by sliding a window of width $t$
        along that flight's trajectory $\mathbf{r}^{(k)}(\cdot)$ and averaging over all
        starting times $t_0$ \emph{within that same flight},
        \begin{equation}
          \overline{\delta^2}^{(k)}(t)\equiv
          \frac{1}{T_k-t}\int_0^{T_k-t}
          \big|\mathbf{r}^{(k)}(t_0+t)-\mathbf{r}^{(k)}(t_0)\big|^2\,\mathrm{d}t_0 .
        \end{equation}
        This internal time average (over $t_0$ within flight $k$) is entirely distinct from
        the ensemble average $\langle\cdots\rangle$ over different flights used in the MSD.
        A single-flight curve $\overline{\delta^2}^{(k)}(t)$ is noisy; the quantity to compare
        with the MSD is the ensemble mean of the time-averaged MSD,
        \begin{equation}
          \mathrm{TAMSD}(t,T)\equiv
          \Big\langle\overline{\delta^2}^{(k)}(t)\Big\rangle_{T_k\approx T},
        \end{equation}
        i.e.\ $\overline{\delta^2}^{(k)}$ averaged over all flights of comparable duration $T$.
        Two distinct signatures of \emph{ergodicity breaking} can then appear.
        \emph{(i) Aging:} $\mathrm{TAMSD}(t,T)$ differs from $\mathrm{MSD}(t)$ and retains a
        systematic dependence on $T$, so flights of different duration yield different curves.
        \emph{(ii) Non-self-averaging:} even as $T\to\infty$ the individual values
        $\overline{\delta^2}^{(k)}(t)$ do not concentrate around a single value -- their
        distribution across trajectories remains broad and does not collapse to a delta function,
        unlike what happens for an ergodic process. The $T$-dependence of
        $\mathrm{TAMSD}(t,T)$ is a sufficient but not necessary condition for non-ergodicity:
        some non-ergodic processes have a weak $T$-dependence while the non-self-averaging
        signature persists. For this reason we monitor both the mean $\mathrm{TAMSD}(t,T)$
        and the distribution of $\overline{\delta^2}^{(k)}(t)$ across trajectories at fixed
        $t$ and $T$. The canonical example -- a CTRW with power-law waiting times -- is
        worked out analytically in Appendix~\ref{app:ctrw}.
  \item[Propagator $P(\Delta\mathbf{r},t)$.] The probability distribution of the horizontal
        displacement vector $\Delta\mathbf{r}(t)=\mathbf{r}(t_0+t)-\mathbf{r}(t_0)$ at a fixed
        lag $t$ -- the propagator of the walk. Because each flight starts at the origin, the
        displacement from take-off coincides with the position, so the displacement and the
        position distributions are the same object; the genuine choice is \emph{which}
        projection to study. We will look at the one-dimensional marginal $P(\Delta x,t)$ of a
        Cartesian component (cleanest for a scaling collapse) and at the radial distribution
        $P(\rho,t)$ of $\rho=|\Delta\mathbf{r}|$. A self-similar, anomalous process obeys the
        collapse $P(\Delta x,t)=t^{-H}\,F(\Delta x\,/\,t^{H})$; the shape of the scaling
        function $F$ separates ordinary diffusion (Gaussian) from anomalous transport (heavy,
        power-law or stretched tails).
