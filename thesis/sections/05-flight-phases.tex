...
\label{ch:phases}
\input{generated/segmentation_audit_values}
...
\begin{tikzpicture}[x=1cm,y=1cm,>=Latex,font=\small]
  \draw[rounded corners,draw=figblue] (0,0) rectangle (3.8,2.6);
  \draw[rounded corners,draw=phasesearch] (4.15,0) rectangle (7.95,2.6);
  \draw[rounded corners,draw=figgreen] (8.3,0) rectangle (12.1,2.6);
  \node at (1.9,2.25) {Transition};
  \node at (6.05,2.25) {Search};
  \node at (10.2,2.25) {Climb};
  \draw[figblue,thick,->] (.35,.85) .. controls (1.5,1.1) and (2.5,.9) .. (3.45,1.35);
  \draw[phasesearch,thick,->] (4.5,.7) .. controls (6.6,.6) and (7.2,1.75) .. (5.8,1.65)
  .. controls (4.7,1.5) and (5.4,.8) .. (7.5,1.05);
  \draw[figgreen,thick,domain=0:780,samples=100,variable=\a,->]
    plot ({10.1+.7*cos(\a)+.00045*\a},{1.15+.52*sin(\a)});
  \node[font=\footnotesize] at (1.9,.3) {directed progress};
  \node[font=\footnotesize] at (6.05,.3) {exploratory turns};
  \node[font=\footnotesize] at (10.2,.3) {turning and ascent};
  \draw[->] (3.82,1.35) -- (4.13,1.35);
  \draw[->] (7.97,1.35) -- (8.28,1.35);
  \draw[->] (1.9,0) -- (1.9,-.5) -- (10.2,-.5) -- (10.2,0);
  \node[fill=white,font=\footnotesize] at (6.05,-.5) {search can be skipped};
\end{tikzpicture}
...
\begin{figure}[tb]
 \centering
 \begin{tikzpicture}[>=Latex,node distance=1.5cm,font=\small,
     latent/.style={circle,draw=black!75,minimum size=1.0cm},
     observed/.style={rectangle,draw=black!65,fill=black!6,minimum width=1.05cm,minimum height=.8cm}]
  ...
  \draw[->] (q2) -- node[above] {$A$} (q3);
  \draw[->] (q1) -- node[right] {$b_{Q_{k-1}}$} (y1);
  \draw[->] (q2) -- node[right] {$b_{Q_k}$} (y2);
  \draw[->] (q3) -- node[right] {$b_{Q_{k+1}}$} (y3);
  \node[left=.4cm of q1,align=right] {hidden\\states};
  \node[left=.4cm of y1,align=right] {observed\\features};
 \end{tikzpicture}
...
\begin{figure}[tb]
 ...
 \begin{tikzpicture}[x=1cm,y=1cm,font=\small]
  \foreach \x/\title in {0/One direction,4.3/Balanced reversal,8.6/Unequal reversal}{
   \draw[black!40,densely dotted] (\x,.925) -- (\x+3.2,.925);
   \draw[black!55,->] (\x,0) -- (\x+3.45,0);
   \draw[black!55,->] (\x,0) -- (\x,2.1);
   \node[above] at (\x+1.7,2.15) {\title};
   \foreach \u/\labeltime in {0/0,1.6/15,3.2/30}{\draw (\x+\u,-.02)--(\x+\u,-.08) node[below,font=\footnotesize] {\labeltime};}
   \node[below,font=\footnotesize] at (\x+1.65,-.45) {Time in window (s)};
  }
  \draw[figblue,thick] (0,1.65)--(3.2,1.65);
  \draw[figblue,thick] (4.3,1.65)--(5.9,1.65)--(5.9,.2)--(7.5,.2);
  \draw[figblue,thick] (8.6,1.65)--(11,1.65)--(11,.2)--(11.8,.2);
  \node[left,font=\footnotesize] at (0,1.65) {$+6$};
  \node[left,font=\footnotesize] at (0,.925) {$0$};
  \node[left,font=\footnotesize] at (0,.2) {$-6$};
  \node[rotate=90,font=\footnotesize] at (-.8,1.0) {$\omega$ (deg/s)};
  \node[below] at (1.6,-.95) {$C_\omega=1$};
  \node[below] at (5.9,-.95) {$C_\omega=0$};
  \node[below] at (10.2,-.95) {$C_\omega=1/2$};
 \end{tikzpicture}
...
\input{generated/segmentation_audit_table}
