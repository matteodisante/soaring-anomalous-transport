"""Fit altitude-band effective scaling exponents with whole-site-day intervals.

Reuse identified segment TA-MSDs and the completed grouped report's flight
membership. No mounted SSD, raw trajectory pass, or historical report edit is
needed. These are range-dependent second-moment exponents, not an identification
...
def measure(args):
    """Check inherited inputs and refit each complete cluster resample."""
    ...
    for discipline in DISCIPLINES:
        lags, members, values = reconstruct(parent, discipline, args.audit_dir, inputs)
        groups = {}
        for number, name in enumerate(parent["results"][discipline]["altitude_band"]):
            mask = members.altitude_band.eq(name).to_numpy()
            ...
            # Independently check agreement with every inherited descriptive fit.
            old = parent["results"][discipline]["altitude_band"][name]
            for fit in summary["fits"]:
                matches = [
                    item
                    for item in old["decade_slopes"][fit["control_index"]]
                    if item["requested_range_s"] == fit["requested_range_s"]
                ]
                if matches:
                    expected_slope = matches[0]["slope"]
                else:
                    parent_curve = np.asarray(old["mean_m2"])[fit["control_index"]]
                    selected = np.isin(lags, fit["actual_lags_s"])
                    expected_slope = np.polyfit(
                        np.log(lags[selected]), np.log(parent_curve[selected]), 1
                    )[0]
                np.testing.assert_allclose(fit["slope"], expected_slope, atol=1e-12)
            summary["n_flights_total"] = int(mask.sum())
            groups[name] = summary
            print(
                f"{discipline}: {name}: {mask.sum():,} flights; fits complete",
                flush=True,
            )
        results[discipline] = {"lags_s": lags, "altitude_band": groups}
    sources = [
        Path(__file__),
    ...
    return portable(
        {
            ...
            "validation": (
                "All altitude means, support and fixed identities match parent; "
                "slopes match inherited fits or an independent fit of parent means"
            ),
            "contract": {
                ...
                "resamples": args.resamples,
                "base_seed": args.seed,
                "pairing": "Same draws across lags and controls within each band",
                "interval_support": (
...
def render(report, out):
    ...
    fig.suptitle("Horizontal TA-MSD by initial GNSS altitude", fontsize=11, y=0.98)
    fig.text(
        0.5,
        0.947,
    ...
    for column, (discipline, entry) in enumerate(report["results"].items()):
        ...
        for control in range(2):
            ...
            ax.set(
                title=f"{discipline.capitalize()}: {population}",
                xlim=(10, 10000),
                ylim=(2e3, 4e10),
...
def save_thesis_assets(report, figures, out):
    ...
    lines = [
        ...
        r"\begin{tabular}{@{}lcc@{}}",
        r"\toprule",
        r"Initial-altitude band & Available segments & Fixed long segments \\",
        r"\midrule",
    ]
    lag_counts = set()
    for discipline, entry in report["results"].items():
        heading = discipline.capitalize()
        lines.extend(
            [
                rf"\multicolumn{{3}}{{@{{}}l}}{{\textit{{{heading}}}}} \\",
                r"\addlinespace[2pt]",
            ]
        )
        for name, row in entry["altitude_band"].items():
