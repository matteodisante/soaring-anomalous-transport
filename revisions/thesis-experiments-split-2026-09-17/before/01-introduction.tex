...
Chapters~3 and~4 of \emph{Esperimenti} therefore measure the MSD, displacement
...
and closed tasks help expose the effect of the flight's intended geometry. Chapter~7
then confronts these and further measurements, the spectrum of moments, the shape of the
increment distribution and velocity correlations, with explicit model predictions: higher
...
legs~\cite{zaburdaev2015}. Chapter~7 specifies the hypotheses being tested and
leaves unsupported exclusions open.
...
of persistence.
Chapter~5 addresses this missing link with a probabilistic segmentation into
...
The present document contains the introduction and the data chapter, together with
the two appendices cited in Chapter~2. The companion volume \emph{Esperimenti}
contains Chapters~3--7 and Appendix~4.A; references to those chapters and their
sections or figures refer to that volume. The chapters on conditional observables, social flight, stochastic
modelling and route optimisation form the research programme; their results are not
claimed here. The immediate priority is to consolidate Chapters~3,
4 and~5, then undertake the solo--group comparison with the
necessary observables conditioned on phase.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[schematic, node distance=5.5mm,
    ...
    \node[done, below=of data] (global) {{\bfseries Chapters 3--4: global observables and transport}\\[2pt]
      {\footnotesize\color{black!70} Scaling, quantile shape and environmental geometry}};
    \node[done, below=of global] (phases) {{\bfseries Chapter 5: flight phases}\\[2pt]
    ...
    \node[fig note, rotate=90, anchor=south]
      at ($($(data.north west)!0.5!(phases.south west)$)+(-6mm,0)$) {thesis and experiments};
    \draw[decorate, decoration={brace, amplitude=4pt}, black!45, line width=0.6pt]
  ...
  \end{tikzpicture}
  \caption{Logical structure of the thesis. Solid boxes are developed in the thesis and its companion volume;
...
including the treatment of missing or inconsistent channels. Chapters~3
and~4 then ask what can be learned without assigning phases.
...
work: the thesis and its companion volume establish the empirical measurements and validation needed
