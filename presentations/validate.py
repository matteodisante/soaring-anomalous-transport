...
def main() -> None:
    ...
    result = {"verified_utc": datetime.now(UTC).isoformat(),
              ...
              "visual_review": "The numerical figures retain their reviewed layouts. Chapter 3 slides 9--10 and their notes were checked for readable formulas, descriptive D2 ranges, the distinction between an analytical persistence example and a fitted model, and the limits of fixed-flight support. Previously reviewed wind slides are now 70--74; their figures and numerical content are unchanged. TAMSD row crops preserve complete axis labels and omit neighbouring rows. Values, support and group definitions agree with the complete reviewed reports."}
