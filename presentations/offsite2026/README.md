...
28 slides total, including the cover, closing research agenda, sources, bibliography, thanks and three appendices, entirely in English. Scope: the current
...
in the thermal-plane viewer. Only slides 4–6 cover preprocessing.
...
- `offsite-2026-notes.pdf`: the same 28 slides, with English speaker notes alongside.
...
- `render_thermal_panels.py`: exports slide 16 maps directly from the viewer's
...
- `measure_cadence_support.py`: measures the cadence-limited extension of C10000
  for slide 9; writes the portable `cadence-support-report.json`.
...
6. Preprocessing steps 6--7: short-gap interpolation versus long-gap splitting, followed by the moving cubic Savitzky--Golay fit, with position and analytic derivatives evaluated at the window centre; no subsequent finite differences.
7. Displacement and equal-flight MSD estimator, with admissible origins and gaps.
8. Why C10000 is selected once: the same flights and segments at every lag from 10 to 10,000 s, with the measured duration-selection shift.
9. C10000 from 1 s, with growing cadence support below 10 s and fixed support above; fitted growth over 10–10,000 s is superdiffusive.
10. Terrain (launch-altitude proxy), experience (Beginners = EN A/B/C, Experts = EN D/CCC) and circuit type; marginal versus stratified comparisons and the distinction from experimental OFAT.
11. Open versus closed circuits: slower growth for return routes, linked to the displacement reduction caused by turns and returns.
...
19. Work in progress: continuous-observation HMM across cadences, anisotropy and distribution scaling.
20. Proposed two-phase transport model: gliding generates displacement; contiguous search and thermalling episodes merge into one waiting phase. Estimate total waiting times, glide lengths/durations and directional memory, then test the 2D-plus-time model with Monte Carlo simulations. Vilpellet's phase-resolved observations motivate the reduction; neglecting motion during waiting remains an approximation to test.
21. Solo/grouped contrasts and interactions in joint factorial models; exposure-aware lift maps and routing under an explicit objective.
22. Flight, terrain, map and analysis sources with clickable links.
23. Selected bibliography.
24. Take-home messages and open questions.
25. Thanks and discussion.
26. Appendix: quantitative factorial model, six additive and ten interaction coefficients, and editable OFAT/full-factorial grids.
27. Appendix: vector velocity autocorrelation as speed-weighted directional agreement; unit-vector correlation and the distinction from mean drift.
28. Appendix: cluster bootstrap with replacement and limitations of current bands.
...
Questions guide slides 8, 10, 11, 12, 14, 15, 16 and 17, where the audience needs
...
Slide 9 keeps C10000 as its parent population and its selected segment identities.
...
The schematic on slide 17 is `assets/thermal-mechanism-3d.png`; its
