...
\begin{frame}{France: launch density and regional selection boxes}
\begin{columns}[c,onlytextwidth]
\column{.55\textwidth}
\fig[57mm]{supervisor-map-france.pdf}
\column{.42\textwidth}\small
Retained paragliders and hang gliders are pooled. The map window contains
94.8\% of their recorded starting locations.
\par\medskip
Colour gives flights per $0.15^\circ\!\times0.15^\circ$ cell, on a logarithmic scale.
\par\medskip
Dashed boxes assign regional membership from the starting location:
Alps, Pyrenees, Massif Central and Channel Coast.
\par\medskip
The PCA below compares Alps, Pyrenees and Channel Coast. Routes may leave their
starting box.
\end{columns}
\source{Chapter 2 geographic-coverage map, France panel. Boxes are latitude/longitude selections; they do not trace terrain boundaries or route exposure.}
\question{Would membership based on the full route change the regional comparison?}
\speaker{The colour scale counts retained flights, not distinct sites or time spent in a cell. The percentage refers to the plotted window, which includes neighbouring countries, not an administrative definition of France. Box bounds (longitude; latitude, degrees) are Alps: 5.4--10.0; 43.8--46.6, Pyrenees: -1.9--3.3; 42.0--43.5, Massif Central: 1.8--4.6; 43.6--46.2, and Channel Coast: -1.8--2.0; 48.3--51.2. The map pools both disciplines, whereas the regional PCA panels shown next concern paragliders. The coastal box includes cliffs and valleys, so its name does not establish low relief along a flight.}
\end{frame}

\begin{frame}{France: recorded starting-altitude bands}
\begin{columns}[c,onlytextwidth]
\column{.53\textwidth}
\fig[58mm]{terrain_map.pdf}
\column{.44\textwidth}\footnotesize
First raw IGC altitude $z_0$: GNSS when usable, with pressure fallback for this map.
\par\smallskip
\begin{tabular}{@{}ll@{}}\toprule
Map label & Recorded altitude\\\midrule
Plains & $z_0<300$ m\\
Hills & $300\le z_0<800$ m\\
Low mountains & $800\le z_0<1500$ m\\
High mountains & $z_0\ge1500$ m\\\bottomrule
\end{tabular}
\par\smallskip
Each $0.15^\circ\!\times0.15^\circ$ cell: colour shows the most frequent altitude
band; marker area grows with log flight count.
\par\smallskip
Crosses mark the ten busiest cells in each band. Both disciplines are pooled.
\par\smallskip
These labels describe starting altitude; they do not measure slope or local relief.
\end{columns}
\source{Chapter 2 starting-altitude map. Raw first-fix proxy with uncertain vertical datum; the independent ETOPO terrain map is introduced later.}
\question{Would verified launch elevations and local relief change the altitude classification?}
\speaker{The map uses the first valid raw B record of each retained flight, before trajectory cleaning or trimming. GNSS is preferred when finite, nonzero and inside the configured plausibility range; otherwise a usable pressure altitude is used. Flights without a usable first altitude do not enter the coloured cells. The pressure fallback is specific to this exploratory map: the cleaned trajectory uses GNSS altitude. Marker area is 6 + 22 log10(n + 1), where n counts classified flights in the cell, and the colour is the modal band rather than the mean altitude. A first fix may precede take-off or occur after flight has begun. The four labels are altitude bins, not terrain-model classes; low starting altitude can coexist with cliffs or steep slopes.}
\end{frame}
