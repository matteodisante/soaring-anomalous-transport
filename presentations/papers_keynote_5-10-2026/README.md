...
| `arakawa-schubert1974.pdf`, `-notes.pdf` | 6-slide deck, and the same deck with speaker notes |
...
The decks stay inside the papers: blue boxes report what the paper says, and each deck ends on the paper's own conclusions and limits.
The reading guides still carry gold boxes and `[ours]` for our proposals and readings, which are not in the papers.

Rebuild:

```
python3 crop_figures.py   # figures cropped from the PDFs into assets/
python3 build.py          # decks, notes and guides; checks page counts 5, 6, 2, 4
```
