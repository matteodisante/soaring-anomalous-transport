...
> Historical working proposal, retained as discussion material. Its descriptions of
> the old cleaner, numerical thresholds, examples and proposed physical interpretations
> do not define the current pipeline. The reviewed methods are in Chapter 2 and
> [the preprocessing guide](docs/guide/preprocessing-pipeline.md).
...
short instrument-vs-weather note in `sec:altchannel`, or go. Note the removed block also carries
...
label. Either give it a home in `sec:altchannel` or drop the step.
...
the retired duration report). Commit and push everything, including the untracked figure
